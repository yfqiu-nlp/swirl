...
class LiquidTextTokenLogitsProcessor(LogitsProcessor):
    """
    Strictly forces the model to generate ONLY tokens within the TEXT vocabulary range.
    Range: [0, 256000)
    """
    def __init__(self, image_start_idx: int = 256000):
        self.image_start_idx = image_start_idx
        self.filter_value = -float("inf")

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        if scores.shape[1] > self.image_start_idx:
            scores[:, self.image_start_idx:] = self.filter_value
        return scores
...
class LiquidGRPOTrainer(GRPOTrainer):
    def __init__(self, processing_class, reward_funcs, data_collator: Optional[DataCollator] = None, **kwargs):
        ...
        self.max_gen_len = getattr(self.args, "max_completion_length", 128)
        self.our_temperature = getattr(self.args, "our_temperature", 0.8)
        self.our_top_p = getattr(self.args, "our_top_p", 0.9)
        self.our_top_k = getattr(self.args, "our_top_k", None)

        self.logits_processor = LogitsProcessorList([
            LiquidTextTokenLogitsProcessor(image_start_idx=256000)
        ])
    ...
    def _generate_and_score_completions(
        ...
        source_images = inputs["source_image_ids"]
        target_images = inputs["target_image_ids"]

        batch_size = prompt_ids.shape[0]
        num_generations = self.num_generations
        chunk_size = getattr(self.args, "chunk_size", 4)
        ...
        while generations_generated < num_generations:
            ...
            gen_kwargs = {
                ...
                "do_sample": True,
                "temperature": self.our_temperature,
                "top_p": self.our_top_p,
                "top_k": self.our_top_k,
        ...
        for ids, logs in zip(all_completion_ids, all_completion_log_probs):
            curr_len = ids.size(1)
            if curr_len < max_len:
                diff = max_len - curr_len
                padded_ids.append(F.pad(ids, (0, diff), value=pad_token_id))
                padded_log_probs.append(F.pad(logs, (0, diff), value=0.0))
            else:
                padded_ids.append(ids)
                padded_log_probs.append(logs)

        completion_ids = torch.cat(padded_ids, dim=0)
        completion_log_probs = torch.cat(padded_log_probs, dim=0)

        completion_mask = (completion_ids != pad_token_id).float()
        ...
        for src, tgt in zip(source_images, target_images):
            for _ in range(num_generations):
                repeated_source.append(src)
                repeated_target.append(tgt)
        ...
        for i in range(0, total_items, chunk_size):
            ...
            src_chunk = repeated_source[i : i + chunk_size]
            tgt_chunk = repeated_target[i : i + chunk_size]

            chunk_full_ids = torch.cat([p_chunk, c_chunk], dim=1)
            chunk_c_mask = completion_mask[i : i + chunk_size]
            ...
            r = self.ref_model_provider(
                source_image_ids=src_chunk,
                target_image_ids=tgt_chunk,
                completions=c_chunk
            )
        ...
        if self.accelerator.is_main_process:
            ...
            if self.state.global_step % self.args.logging_steps == 0:
                print(f"[Rollout Debug] Reward: {rewards[0].item():.4f}")
                try:
                    text = self.processing_class.decode(completion_ids[0], skip_special_tokens=True)
                    print(f"[Rollout Debug] Text: {text}")
                except: pass
